...
@given(
    parsers.re(
        "I have an EXO user mailbox with the following attributes:\n?(?P<content>.*)",
        flags=re.DOTALL,
    )
)
def create_exo_user_mailbox(
    content: str, exo_mailboxes: dict[EmailAddress, ExoMailboxWithPermissions]
):
    parsed = parse_key_val_lines(content)
    gherkin = GherkinExoUserMailbox.model_validate(parsed)
    ...
    mailbox = ExoMailbox(
        ...
        ExternalDirectoryObjectId=gherkin.ExternalDirectoryObjectId,
        Guid=gherkin.Guid,
        RecipientTypeDetails=RecipientTypeDetailsEnum.USER_MAILBOX,
    )
    ...
    if mailbox.PrimarySmtpAddress in exo_mailboxes:
        logger.warning(
            f"EXO user mailbox {mailbox.PrimarySmtpAddress} already exists in the mailboxes. "
